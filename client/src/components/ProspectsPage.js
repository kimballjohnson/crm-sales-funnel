...
function ProspectsPage({prospects, loading, companies, addingProspect, setAddingProspect, setAddingCompany}) {
    ...
    const prospectsToDisplay = prospects.filter(prospect => 
        prospect.first_name.toLowerCase().includes(search.toLowerCase()) 
        || prospect.last_name.toLowerCase().includes(search.toLowerCase()) 
        // || prospect.company_id.name.toLowerCase().includes(search.toLowerCase) 
        // || prospect.stage.toLowerCase().includes(search.toLowerCase)
        )
    ...
    return(
        <Page>
            ...
            <ProspectCard key={prospect.id} prospect={prospect} companies={companies}/>
                ...
                {addingProspect ? null : <Button onClick={() => setAddingProspect(!addingProspect)}>Add a Prospect</Button>}
                {!addingProspect ? null : <NewProspectForm setAddingProspect={setAddingProspect} setAddingCompany={setAddingCompany} companies={companies}/>}
